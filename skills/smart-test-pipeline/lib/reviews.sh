...
capture_review_baseline() {
  ...
  : > "$baseline_file"
  for bot in $bots; do
    local login review_id issue_id review_comments issue_comments
    case "$bot" in
...
is_review_bot_done() {
  local bot="$1" owner="$2" repo="$3" pr_num="$4" baseline_file="$5" login review_baseline issue_baseline review_comments issue_comments review_bodies issue_bodies
  case "$bot" in
    coderabbit) login="coderabbitai[bot]" ;;
    greptile) login="greptile-apps[bot]" ;;
    *) return 1 ;;
